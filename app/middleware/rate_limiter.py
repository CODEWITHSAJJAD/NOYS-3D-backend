...
class RateLimiter(BaseHTTPMiddleware):
    """
    Production-grade rate limiter with multiple tier support
    - Different limits for different endpoint types
    - Sliding window algorithm for accuracy
    - IP + User based limiting for authenticated routes
    """
    ...
    def _get_client_id(self, request: Request) -> str:
        ...
        if auth_header and auth_header.startswith("Bearer "):
            # Use IP + first 10 chars of token as key
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Stricter for generation endpoints
        ...
        # Use sliding window for this specific endpoint
...
class CacheMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Skip for authenticated/admin endpoints and list endpoints
        path = request.url.path
